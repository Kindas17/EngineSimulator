...
int main(int argc, char *argv[]) {
  ...
  while (game.isGameRunning()) {
    ...
    ImGui::Begin("Test");
    ImGui::SliderFloat(
        "External Torque [Nm]", &piston.externalTorque, 0.f, 20.f);
    ImGui::SliderFloat(
        "Intake Coef", &piston.intakeCoef, 0.00012f, 0.0012f, "%.6f");
    ImGui::SliderFloat(
        "Exhaust Coef", &piston.exhaustCoef, 0.00008f, 0.0008f, "%.6f");
    ...
    ImPlot::PlotLine(
        "Exhaust Flow", outflowLog.getData(), outflowLog.getSize());
    ImPlot::EndPlot();
    ImGui::End();
    ...
    ImPlot::BeginPlot("ASD");
    ImPlot::PlotLine(
        "Intake Temp", intakeTempLog.getData(), intakeTempLog.getSize());
    ImPlot::PlotLine(
        "Exhaust Temp", exhaustTempLog.getData(), exhaustTempLog.getSize());
    ImPlot::EndPlot();
    ImGui::End();
    ...
    ImPlot::BeginPlot("ASD");
    ImPlot::PlotLine(
        "Intake Pressure", intakePresLog.getData(), intakePresLog.getSize());
    ImPlot::PlotLine(
        "Exhaust Pressure", exhaustPresLog.getData(), exhaustPresLog.getSize());
    ImPlot::EndPlot();
    ImGui::End();
