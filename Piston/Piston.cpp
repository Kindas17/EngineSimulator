...
inline float adiabaticCompression(float initialVolume, float initialPressure, float currentVolume)
{
...
void Piston::updatePosition(float deltaT, float setSpeed)
{
    ...
    headAngle += RADToDEG(omega) * deltaT / 2; /* Head crank rotates at half the speed */
...
void Piston::applyExtTorque(float torque)
{
...
void Piston::updateStatus(float deltaT)
{
    ...
    // // combustionAdvance = (RADSToRPM(omega) < 2500) ? 7.5 * RADSToRPM(omega) / 2500 : 7.5;

    // const float intake_k_flow = intakeValve * (throttle * (1 - minThrottle) + minThrottle);

    // intakeFlow = intakeOri.flow(intake_k_flow * 0.2f * thermo.gas.nR, thermo.gas.P);
    // exhaustFlow = exhaustOri.flow(exhaustValve * 0.2f * thermo.gas.nR, thermo.gas.P);
    // const float nR_prime = intakeFlow + exhaustFlow;
    ...
    // // intakeFlow = thermo.orificeFlow(deltaT, intake_k_flow, 0.25, 300, 101325, 1);
    // // exhaustFlow = thermo.orificeFlow(deltaT, exhaustValve * coef, 0.25,  300, 101325, 0);
    // // leakageFlow = thermo.orificeFlow(deltaT, 0.00001f * coef, 0.01, 300, 101325, 0);
...
void Piston::ValveMgm()
{
    ...
    const float x_int = (angleWrapper(headAngle - 180) - (45+180)) / profileSpeed1;
    intakeValve = expf(-(x_int * x_int));

    /* Exhaust Profile */
    const float profileSpeed2 = 20;
    const float x_exh = (angleWrapper(headAngle + 180) - (315-180)) / profileSpeed2;
...
float Piston::getPistonPosition()
{
    ...
    const float b = (geometry.stroke/2) * (geometry.stroke/2) * cosf(DEGToRAD(currentAngle)) * cosf(DEGToRAD(currentAngle));
...
{
    return (- getPistonPosition() - geometry.rod + geometry.stroke/2) / (geometry.stroke);
}

float Piston::getChamberVolume()
{
    const float constantVol = geometry.bore * geometry.bore * M_PI * 0.25 * geometry.addStroke;
    return (1 - getCyclePercent()) * M_PI * geometry.bore * geometry.bore * geometry.stroke * 0.25 + constantVol;
}

float Piston::getMaxVolume()
{
    return M_PI * (geometry.bore) * (geometry.bore) * (geometry.addStroke + geometry.stroke) * 0.25;
}
...
{
    return getMaxVolume() / (geometry.bore * geometry.bore * M_PI * 0.25 * geometry.addStroke);
}

float Piston::getThetaAngle()
{
    return asinf(geometry.stroke/(2*geometry.rod) * cosf(DEGToRAD(currentAngle)));
}
...
float Piston::getTorque()
{
    ...
    // const float force = pistonSurface * (topPistonPressure - 101325) * cosf(getThetaAngle());
