...
{
    return (((A.x-C.x)*(C.y-D.y) - (A.y-C.y)*(C.x-D.x)) / ((A.x-B.x)*(C.y-D.y) - (A.y-B.y)*(C.x-D.x)));
}
...
{
    return (((A.x-C.x)*(A.y-B.y) - (A.y-C.y)*(A.x-B.x)) / ((A.x-B.x)*(C.y-D.y) - (A.y-B.y)*(C.x-D.x)));
}

inline float lineToPointDistance(vector2_T A, vector2_T B, vector2_T P)
{
    const float num = fabsf((B.x - A.x) * (A.y - P.y) - (A.x - P.x) * (B.y - A.y));
    const float den = sqrtf((B.x - A.x)*(B.x - A.x) + (B.y - A.y)*(B.y - A.y));
    return num/den;
}
